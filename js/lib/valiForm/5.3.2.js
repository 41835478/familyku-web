...
(function(f, b) {
	...
	var a = function(l, n, k) {
		var n = $.extend({}, a.defaults, n);
		n.datatype && $.extend(a.util.dataType, n.datatype);
		var m = this;
		m.tipmsg = {
			w: {}
		};
	...
	a.util = {
		dataType: {
			...
			m: /^(13[0-9]|15[012356789]|17[01678]|18[0-9]|14[57])[0-9]{8}$/,
			e: /^\w+([-+.']\w+)*@\w+([-.]\w+)*\.\w+([-.]\w+)*$/,
			url: /^(\w+:\/\/)?\w+(\.\w+)+.*$/
		},
		toString: Object.prototype.toString,
		isEmpty: function(k) {
			return k === "" || k === $.trim(this.attr("tip"))
		},
		getValue: function(m) {
			var l, k = this;
			if (m.is(":radio")) {
		...
		enhance: function(l, m, n, k) {
			...
			o.find("input[recheck]").each(function() {
				...
				this.validform_inited = "inited";
				var q = $(this);
				var p = o.find("input[name='" + $(this).attr("recheck") + "']");
		...
		usePlugin: function(o, l, n, r) {
			...
			if (s.find("input[plugin='datepicker']").length && $.fn.datePicker) {
				...
				s.find("input[plugin='datepicker']").each(function(t) {
					...
					this.validform_inited = "inited";
					o.datepicker.callback && $(this).bind("dateSelected", function() {
						var u = new Date($.event._dpCache[this._dpId].getSelected()[0]).asString(Date.format);
						o.datepicker.callback(u, this)
					});
					$(this).datePicker(o.datepicker)
				})
			}
			if (s.find("input[plugin*='passwordStrength']").length && $.fn.passwordStrength) {
				...
				s.find("input[plugin='passwordStrength']").each(function(t) {
					...
					this.validform_inited = "inited";
					$(this).passwordStrength(o.passwordstrength)
				})
			}
			if (r != "addRule" && o.jqtransform && $.fn.jqTransSelect) {
				if (s[0].jqTransSelected == "true") {
					return
				}
				s[0].jqTransSelected = "true";
				var m = function(t) {
					var u = $(".jqTransformSelectWrapper ul:visible");
					u.each(function() {
						var v = $(this).parents(".jqTransformSelectWrapper:first").find("select").get(0);
				...
				var p = function(t) {
					if ($(t.target).parents(".jqTransformSelectWrapper").length === 0) {
						m($(t.target))
					}
				};
				var q = function() {
		...
		getErrormsg: function(s, n, u) {
			...
			var p = $.extend({}, e.w, s.data("tipmsg").w);
			if (q[0] in p) {
		...
		check: function(v, t, n) {
			var o = v[0].settings;
			var t = t || "";
			var k = a.util.getValue.call(v, $(this));
			if (o.ignoreHidden && $(this).is(":hidden") || $(this).data("dataIgnore") === "dataIgnore") {
				return true
			}
			if (o.dragonfly && !$(this).data("cked") && a.util.isEmpty.call($(this), k) && $(this).attr("ignore") != "ignore") {
				return false
			}
			var s = a.util.regcheck.call(v, $(this).attr("datatype"), k, $(this));
			if (k == this.validform_lastval && !$(this).attr("recheck") && t == "") {
			...
			var q = $(this).attr("ajaxurl");
			if (q && !a.util.isEmpty.call($(this), k) && !n) {
				var m = $(this);
				if (t == "postform") {
				...
				a.util.abort.call(r[0]);
				var u = $.extend(true, {}, o.ajaxurl || {});
				var p = {
					type: "POST",
					cache: false,
					url: q,
					data: "param=" + encodeURIComponent(k) + "&name=" + encodeURIComponent($(this).attr("name")),
					success: function(x) {
						if ($.trim(x.status) === "y") {
				...
				r[0].validform_ajax = $.ajax(u);
				return "ajax"
			} else {
				if (q && a.util.isEmpty.call($(this), k)) {
					a.util.abort.call(r[0]);
		...
		submitForm: function(o, l, k, r, t) {
			...
			w.find("[datatype]").each(function() {
				...
				var z = a.util.getValue.call(w, $(this)),
					A;
				g = A = $(this);
				n = a.util.regcheck.call(w, $(this).attr("datatype"), z, $(this));
				if (!n.passed) {
				...
				if ($(this).attr("ajaxurl") && !a.util.isEmpty.call($(this), z)) {
	...
	a.prototype = {
		...
		eq: function(l) {
			...
			if (!(l in k.objects)) {
				k.objects[l] = new a($(k.forms[l]).get(), {}, true)
			}
			return k.objects[l]
		},
		resetStatus: function() {
			var k = this;
			$(k.forms).each(function() {
				this.validform_status = "normal"
			});
			return this
		},
		...
		getStatus: function() {
			var l = this;
			var k = $(l.forms)[0].validform_status;
			return k
		},
		ignore: function(k) {
			var l = this;
			var k = k || "[datatype]";
			$(l.forms).find(k).each(function() {
		...
		addRule: function(n) {
			...
			for (var l = 0; l < n.length; l++) {
				var p = $(m.forms).find(n[l].ele);
				for (var k in n[l]) {
					k !== "ele" && p.attr(k, n[l][k])
				}
			}
			$(m.forms).each(function() {
				var o = $(this);
		...
		ajaxPost: function(k, m, l) {
			var n = this;
			$(n.forms).each(function() {
				if (this.settings.tiptype == 1 || this.settings.tiptype == 2 || this.settings.tiptype == 3) {
					c()
				}
				a.util.submitForm.call($(n.forms[0]), this.settings, k, l, "ajaxPost", m)
			});
			return this
		},
		submitForm: function(k, l) {
			var m = this;
			$(m.forms).each(function() {
				var n = a.util.submitForm.call($(this), this.settings, k, l);
				n === b && (n = true);
				if (n === true) {
					this.submit()
				}
		...
		config: function(k) {
			...
			$(l.forms).each(function() {
				var m = $(this);
				this.settings = $.extend(true, this.settings, k);
				a.util.enhance.call(m, this.settings.tiptype, this.settings.usePlugin, this.settings.tipSweep)
			});
			return this
		}
	};
	$.fn.Validform = function(k) {
		return new a(this, k)
	};

	function h(n, m) {
		var l = ($(window).width() - n.outerWidth()) / 2,
			k = ($(window).height() - n.outerHeight()) / 2,
	...
	function c() {
		...
		j = $('<div id="Validform_msg"><div class="Validform_title">' + e.tit + '<a class="Validform_close" href="javascript:void(0);">&chi;</a></div><div class="Validform_info"></div><div class="iframe"><iframe frameborder="0" scrolling="no" height="100%" width="100%"></iframe></div></div>').appendTo("body");
